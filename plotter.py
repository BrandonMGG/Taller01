...
def plotter():
    files = [txt for txt in glob.glob("resultsTaylor/*.txt")]
    # Log file format
    ...
    for txt in files:
        with open(txt) as fd:
            ...
            threads, runtimes, errors = zip(
                *[(float(match.group(1)), float(match.group(2)), float(match.group(3))) for line in fd.readlines() for
                  match in [pattern.search(line)] if match])
    ...
    fig = make_subplots(rows=3, cols=1,
                        subplot_titles=("Threads and Avg Runtimes", "Threads and Avg Error", 'Threads and Speedup'))
