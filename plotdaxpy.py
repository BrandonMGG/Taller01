...
def plotdaxpy():
    files = [txt for txt in glob.glob("resultsDaxpy/*.txt")]
    # Log file format
    ...
    for txt in files:
        with open(txt) as fd:
            ...
            threads, runtimes = zip(
                *[(float(match.group(1)), float(match.group(2))) for line in fd.readlines() for match in
                  [pattern.search(line)] if match])
